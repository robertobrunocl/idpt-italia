...
La mappa componenti `idpt_components.html` espone **quattro mappe affiancate in CSS grid 2×2** — le 3 componenti separate più l'aggregato finale — per permettere il confronto visuale diretto. Una lettura interessante che emerge dal confronto: D1 (pressione demografica) e D2 (peso economico) hanno gradienti Nord/Sud molto simili (Sud più alto), mentre D3 (eredità storica del retributivo) mostra un gradiente diverso, più legato alla composizione storica del lavoro pubblico nelle singole province che non al gradiente economico complessivo — Sondrio e Belluno, per esempio, mostrano valori D3 alti pur essendo nel Nord, perché hanno alta concentrazione di pensioni di magistrati e impiegati pubblici di carriera lunga. Il limite di questa visualizzazione è che è **uno snapshot 2026**, senza profondità storica: una prima lettura temporale è però fornita dal grafico della serie storica descritto qui sotto, mentre un'animazione della traiettoria dell'IDPT *aggregato* nel tempo (che richiederebbe i denominatori ISTAT storici provinciali, non disponibili) resta un upgrade post-progetto naturale, citato in 5.4.

**Oltre lo snapshot: la serie storica interattiva 1998–2026.** La cartografia è un fermo-immagine, ma il grafo conserva anche la profondità temporale. Il cubo 3 (serie storica delle pensioni per sede INPS, 1998–2026, il cubo più voluminoso del progetto con ~79.000 triple) alimenta un grafico a linee interattivo — `docs/serie_storica.html`, generato da `scripts/build_serie_storica.py` che estrae i dati dal cubo 3 via SPARQL e li inietta in una pagina HTML autocontenuta con Chart.js, nella stessa filosofia "singolo file standalone" delle mappe Folium. Il grafico è embeddato nella versione HTML del report e permette di selezionare singole sedi, alternare numero di pensioni e importo medio mensile, e attivare preset narrativi (totale nazionale, top 5 IDPT, province nate nel 2009, salto Sardegna 2012). Tre letture emergono subito. Il **totale nazionale** sale da 15,2 a 20,9 milioni di pensioni (+37% in 28 anni), una crescita continua che è la cornice macro del progetto. Due marcatori verticali rendono visibili le discontinuità del territorio: al **2009** compaiono Barletta-Andria-Trani, Fermo e Monza-Brianza (prima incluse nelle province madri); al **2012** l'accorpamento delle sedi sarde produce il salto netto di Cagliari-e-Sud-Sardegna (+48.000 in un anno) e Sassari (+30.000). Che il totale nazionale resti *liscio* attraverso entrambi gli eventi conferma visivamente che si tratta di pure ridistribuzioni interne, non di variazioni reali dello stock — l'assunzione su cui poggia l'aggregazione retroattiva di sez. 3.
